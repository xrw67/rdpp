...
int ACE_SSL_SOCK_Connector::net_connect(ACE_SSL_SOCK_Stream &new_stream,
                    const ACE_Addr &remote_sap, const ACE_Time_Value *timeout,
                    const ACE_Addr &local_sap, int reuse_addr, int flags, int perms)
{
    int result =
        this->connector_.connect(new_stream.peer(), remote_sap, timeout,
    ...
    if (new_stream.get_handle() == ACE_INVALID_HANDLE
        && (result == 0
            || (result == -1 && (error == EWOULDBLOCK || error == EINPROGRESS))))
